#ifndef STB__INCLUDE_STB_SEQIO_H
#define STB__INCLUDE_STB_SEQIO_H
...
typedef struct {
  char *dbname;      /* The name of the database being searched (if this is a search of an actual database).*/
  char *filename;    /* The name of the file currently being read. */
  char *format;      /* The format of the file (and the current entry). */
  int entryno;       /* The location of the current entry in the file (if entryno is 10, then the current entry is the tenth entry in the file). */
  int seqno;         /* The location of the current sequence in the current entry (if seqno is 3, then the current sequence is the third in the current entry). */
  int numseqs;       /* The number of sequences contained in the current entry. */
  char *date;        /* A single date giving the last time the entry was either created or updated. Its format should be day-month-year, as in 31-JAN-1995. */ 
  char *idlist;      /* The list of identifiers given in the entry. The idlist's form is a string containing vertical bar separated list of identifiers, each of whose form consists of an identifier prefix, a ':' and the identifier. e.g. acc:AFGB2341.1*/
  char *description; /* A description of the sequence or sequences in the entry. */
  char *comment;     /* A block of text giving a comment about the sequence. The string can contain one or more lines of any length. */
  char *organism;    /* The name of the organism the sequence was taken from. */
  char *history;     /* This holds the lines of text placed in the comment section of entries which describe previous SEQIO operations on this entry, i.e., it holds the history of alterations and updates made to this entry by programs using the SEQIO package. */
  int isfragment;    /* This integer is non-zero if the sequence is a fragment of a larger sequence, and zero if the sequence is complete (or if it is not known whether the sequence is a fragment). */
  int iscircular;    /* This integer is non-zero if the sequence is a circular sequence, and zero if it is a linear sequence (or if it's circularity is not known). */
  int alphabet;      /* This integer is one of the predefined constants DNA, RNA, PROTEIN or UNKNOWN. */
  int fragstart;     /* When the sequence is a fragment of a larger sequence and the location of this fragment in the larger sequence is known, this value gives the starting position of the fragment. If this value is not known (or the sequence is complete), fragstart is set to 0. */
  int truelen;       /* This is the "true" length of the sequence, i.e., the length of the sequence without any gap characters or notational characters. Typically, these are just the alphabetic characters. */
  int rawlen;        /* This is the "raw" length of the sequence, i.e., the length of the sequence which includes the gap and notational characters. Typically these are all characters except whitespace and digits. */
} SEQINFO;
...
/* Initial FastQ support (Only sequence data) */
static int fastq_getinfo(INTSEQFILE *, char *, int, int);
...
/* Initial PDB support */
static int pdb_read(INTSEQFILE *, int), pdb_getseq(INTSEQFILE *, int);
static int pdb_getinfo(INTSEQFILE *, char *, int, int);
...
#define file_table_size 51

static FILE_TABLE file_table[] = {
  ...
  { "FASTQ",  FORMAT_FASTQ,  T_GENERAL,
      "@",
      basic_read,  basic_getseq,  fastq_getinfo,  fasta_putseq,
      fasta_annotate },
  { "PDB",  FORMAT_PDB,  T_ALIGNMENT,
      "HEADER",
      pdb_read,  pdb_getseq,  pdb_getinfo,  genbank_putseq,
      genbank_annotate },
};
...
static int intseqf_open(INTSEQFILE *isfp, char *filename, char *format)
{
  ...
  isfp->format = id = subid = FORMAT_UNKNOWN;
...
static int basic_read(INTSEQFILE *isfp, int flag)
{
  ...
  /* variables needed to properly read a fastq file */ 
  int fastq_seqlength = 0;
  char *fastq_qualstart = NULL;
  int fastq_quallength = 0;
  ...
  switch (format) {
    ...
    /* A fastq entry starts with an @ followed by an identifier */
    case FORMAT_FASTQ:
      keych = '@';
      break;
  ...
  /* make sure we are at the first header line */
  while ((status = fp_get_line(isfp, &line, &end)) == STATUS_OK) {
    if (line[0] == keych) {
      break;
    }
  ...
  /* If we did not find any header lines in the file, the file does not contain any entries */
  if (status == STATUS_EOF) {
  ...
  if (status == STATUS_OK) {
    switch (format) {
      /* Fasta files can contain several lines with comments (starting with an ;), 
       * we will skip past those and move to the start of the sequence data 
       */
      case FORMAT_FASTA:
      case FORMAT_FASTAOLD:
        if (line[0] == ';') {
  ...
  if (status == STATUS_OK && line[0] != keych) {
    ...
    switch (format) {
      ...
      /* The fastq file format is a bit tricky to read:
       * An entry starts with an @ followed by the sequence data and then a + followed by information about the sequence quality.
       * However, the quality data can also contain a @ character, so we cannot simple read untill we find another @, because it might still be
       * part of the quality data (see Example). Instead we need to keep track of the length of the sequence data and make sure that the length 
       * of the sequence data matches the length of the quality data (e.a. every basepair had a quality score). Only then, we know we have read 
       * the entire entry and the @ is the start of the next sequence.
       * 
       * Example:
       * @EAS54_6_R1_2_1_413_324   <- Header line
       * CCCTTCTTGTCTTCAGCGTTTCTCC <- Sequence data
       * GAC                       <- Similar to a fasta file, the sequence data can be spreak accross several lines
       * +                         <- The start of the quality data (optionally contains the same identifier as the header line)
       * ;;3;;;@;;;;;;;;7;;;;;;;88 <- Start of the Quality data
       * @)-                       <- Eventhough the line starts with an @ there is still more quality data and the @ is not the start of the
       *                              next entry
      ...
      case FORMAT_FASTQ:
        while (status == STATUS_OK && line[0] != keych && (fastq_seqlength == fastq_quallength)) {
           ...
           */
          if (line[0] == '+') {
            fastq_seqlength = line - isfp->fp_seqstart;
          }

          /* get another line */
          status = fp_get_line(isfp, &line, &end);
          
          /* If we finished reading the sequence data and we did not determine the start of the quality data, do that now. We need
           * we will need to know the start of the quality data to determine the total length of the quality data
           */
          if (fastq_seqlength && !fastq_qualstart) {
            fastq_qualstart = line;
            fastq_quallength = 0;
          }
          
          /* determine the amount of quality data we have read so far */
          fastq_quallength = line - fastq_qualstart;
...
;
  ...
  /* find the beginning of the sequence start 
   * Name    ACGTACG <- Skip the name and then the spaces
   */
  ...
  while (s < end) {
     ...
     * SEQ1 AGCTAGCTAGT'\n'
     * SEQ2 AGCTAG-TAGT'\n'
     * '\n' <- First move to this line 
     * '\n'
     * '\n' 
     * '\n' <- Second move past all the empty lines (only contain an '\n')
     * SEQ1 AGCT'\n' <- Third count how far we need to move down to get to the next sequence (malign_seqno)
     * SEQ2 A--T'\n'
     * EOF
     */
    for ( ; s < end && (*s != '\n' || !isspace(s[1])); ++s)
    ...
    //printf("End of first block: [%s]\n", s);

    for ( ; s < end && (*s != '\n' || isspace(s[1])); ++s) 
      ;
    
    //printf("Beginning of second block: [%s]\n", s);
    
    for (count=0; s < end && (*s != '\n' || !isspace(s[1])); ++s) {
      if (*s == '\n' && ++count == isfp->malign_seqno) {
        //printf("\nStart of next sequence part: \n[%s]\n", s);
        break;
      }
    }
  }
...
static int pdb_read(INTSEQFILE *isfp, int flag)
{
  int status, count = 0;
  char *end, *line;
  ...
  if (isfp->entry_count == 0) {
    ...
    while ((status = fp_get_line(isfp, &line, &end)) == STATUS_OK) {
      if (line[0] == 'E' && line[1] == 'N' && line[2] == 'D') {
        break;
      } else if (mystreq(line, 'S', "SEQRES") && !isfp->fp_seqstart) {
        isfp->fp_seqstart = line;
      } else if (mystreq(line, 'D', "DBREF ")) {
        count++;
      }
    }
    ...
    switch (status) {
      ...
      case STATUS_EOF:   
        raise_error(E_PARSEERROR, return STATUS_ERROR, print_error("%s:  Empty file.\n", isfp->filename));
...
static char pdb_lookup(char *three_letter_code)
{
  char amino_acids_1letter[20]   = {"ACDEFGHIKLMNPQRSTVWY"};
  char amino_acids_3letter[20][3] = {{"ALA"}, {"CYS"}, {"ASP"}, {"GLU"}, {"PHE"}, {"GLY"}, {"HIS"}, {"ILE"}, {"LYS"}, {"LEU"}, {"MET"}, {"ASN"}, {"PRO"}, {"GLN"}, {"ARG"}, {"SER"}, {"THR"}, {"VAL"}, {"TRP"}, {"TYR"}};

  char dna_1letter[6] = {"ACGITU"};
  char dna_3letter[6][3] = {{" DA"}, {" DC"}, {" DG"}, {" DI"}, {" DT"}, {" DU"}};

  char rna_1letter[6] = {"ACGITU"};
  char rna_3letter[6][3] = {{"  A"}, {"  C"}, {"  G"}, {"  I"}, {"  T"}, {"  U"}};

  int i;
  for (i = 0; i < 20; i++) {
    if (myncasecmp(three_letter_code, amino_acids_3letter[i], 3) == 0) {
      return amino_acids_1letter[i];
    }
  }

  for (i = 0; i < 6; i++) {
    if (myncasecmp(three_letter_code, dna_3letter[i], 3) == 0) {
      return dna_1letter[i];
    }
  }

  for (i = 0; i < 6; i++) {
    if (myncasecmp(three_letter_code, rna_3letter[i], 3) == 0) {
      return rna_1letter[i];
    }
  }

  fprintf(stderr, "Unknown three letter code in PDB file: %s\n", three_letter_code);
  
  return '\0';
}

static int pdb_getseq(INTSEQFILE *isfp, int rawseqflag)
{
  int seqsize;
  char *s, *end, *seq;
  ...
  */
  ...
  char chainid = isfp->fp_seqstart[11];
  int i;
  switch (rawseqflag) {
    case GETSEQ_SEQUENCE:
      for ( ; s < end;) {
        if (!mystreq(s, 'S', "SEQRES")) {
          break;
        }
        if (s[11] != chainid) {
          break;
...
static int fastq_getinfo(INTSEQFILE *isfp, char *entry, int len, int flag)
{
  int allflag, status;
  char *s, *t, *end;
  INFO info;
  ...
  if (*entry != '@') {
    if (isfp->filename && isfp->filename[0]) {
      raise_error(E_PARSEERROR, return STATUS_ERROR,
                  print_error("%s, entry %d:  Invalid format of FASTQ "
                              "entry.\n", isfp->filename, isfp->entry_count));
    }
    else {
      raise_error(E_PARSEERROR, return STATUS_ERROR,
                  print_error("seqfparseent:  Invalid format of FASTQ "
                              "entry.\n"));
    }
  }
...
static int clustal_getinfo(INTSEQFILE *isfp, char *entry, int len, int flag)
{
  ...
  int alignment = 0;
  int length_sequence_name = 0;
  if (allflag || flag == SEQINFO_IDLIST) {
  /* First find the sequence alignment:
   * skip to the end of line and move your way back untill we find the space separating the alignment and the name then move back more untill you find the last character of the sequence name
   */
    s = isfp->fp_seqstart;
    ...
    /* find the start of the sequence name field */
    for (s-=1; s > 0 && !isspace(*s); s--) {
      alignment++;
    }

    /* find the start of the actual sequence name */
    for (s-=1; s > 0 && isspace(*s); s--) {
      alignment++;
    }
    length_sequence_name = line_len - alignment;

    add_id(&info, (isfp->db_idprefix ? isfp->db_idprefix : "oth"), isfp->fp_seqstart, isfp->fp_seqstart+length_sequence_name);
  }
...
static int pdb_getinfo(INTSEQFILE *isfp, char *entry, int len, int flag)
{
  int allflag, status;
  char *start;
  char chain[8] = "Chain A ";
  INFO info;

  start = isfp->fp_entrystart;
  ...
  if (allflag || flag == SEQINFO_DESCRIPTION) {
    chain[6] = chain[6] + isfp->malign_seqno - 1;
    add_description(&info, &chain[0], &chain[7]);
    add_description(&info, &start[10], &start[49]);
    add_id(&info, "pdb", &start[62], &start[66]);
    set_date(&info, &start[50], &start[59]);
  }
...
static int genbank_annotate(FILE *fp, char *entry, int entrylen, char *newcomment, int flag)
{
...
{
#ifdef GZ_SUPPORT
  if ((*ptr_out = gzopen(filename, "r")) >= 0)
    return STATUS_OK;
  else
    return STATUS_ERROR;
#else
...
{
#ifdef GZ_SUPPORT 
  if (gzseek(ptr, bytepos, SEEK_SET) >= 0)
    return STATUS_OK;
  else
    return STATUS_ERROR;
#else
