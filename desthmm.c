...
int main (int argc, char **argv)
{
	...
	dhmm_loadseq(argv[optind], &T, &O); 

	/* initialize the hmm model */
	if (iflg) { 
		dhmm_load(hmminitfile, &hmm);
	} else if (sflg) {
		dhmm_init(&hmm, N, "0123", M, "012", seed);
	} else {
		seed = hmm_seed();
		dhmm_init(&hmm, N, "0123", M, "012", seed);
	}
	...
	/* call Baum Welch */
	dhmm_baumwelch(&hmm, T, O, alpha, beta, gamma, &niter, &logprobinit, &logprobfinal);
	...
	if (vflg) {
		...
		fprintf(stderr, "Log Prob(observation | initial model): %E\n", logprobinit);	
